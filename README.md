...
An example of a download manager is [Tizen's Download API](https://developer.tizen.org/dev-guide/2.2.1/org.tizen.web.device.apireference/tizen/download.html). It allows the developer to issue a download request and define the network types (cellular, wifi, all) in which the download operation can proceed, monitor the state of the download (queued, downloading, paused, canceled, completed, failed), and programmatically start, cancel, pause, or resume the download.
